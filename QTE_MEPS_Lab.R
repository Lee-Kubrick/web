...
mepsdata <- read.csv("/meps1819expandage.csv")
...
x1c<-cbind(mepsdata$logincome)

x2d<-cbind(mepsdata$age,mepsdata$race,mepsdata$sex,mepsdata$region,mepsdata$insure,mepsdata$pain2,mepsdata$pain3,
              mepsdata$marriage,mepsdata$occupation,mepsdata$employ)

x3o<-cbind(mepsdata$educ,mepsdata$pain1,mepsdata$familysize)
...
## CDF function creat v


ccdfv<-function(ws,w,hw,x1c,x1cs,hx1,x2d,x2ds,lamdx2,x3o,x3os,lamdx3){
	xr1<-nrow(x1c)
    xr2<-nrow(x2dg)
    xr3<-nrow(x3og)
	u1<-kronecker(t(x2dgs),array(1,dim=c(xr2,1)))
    u2<-kronecker(t(x2dgs),array(1,dim=c(xr2,1)))
    u3<-kronecker(t(x3ogs),array(1,dim=c(xr3,1)))
    u4<-kronecker(t(ws),array(1,dim=c(length(w),1)))
    hwn<-kronecker(hw,array(1,dim=c(length(w),1)))
	hx1n<-kronecker(t(hx1),array(1,dim=c(xr2,1)))
    lamdx2n<-kronecker(t(lamdx2),array(1,dim=c(xr2,1)))
    lamdx3n<-kronecker(t(lamdx3),array(1,dim=c(xr3,1)))
    out<-sum(pnorm((u4-w)/hwn)*kf((x1c-u1)/hx1n)* dkf(x2d,u2,lamdx2n)*
               dorderdf(x3o,u3,lamdx3n))/sum(kf((x1c-u1)/hx1n)* dkf(x2d,u2,lamdx2n)*
               dorderdf(x3o,u3,lamdx3n))
  return(out)
}


###bandwidth for cdf w on z and x
nw<-length(w)
v<-rep(0,nw)
hw<-1.06*sd(w)/n^(.2)
hx1 <-c(1.06*sd(x1cg)/n^(.2))
lamdx2<-c(1.06*apply(x2d,2,sd)/n^(.2))
lamdx3<-c(1.06*apply(x3o,2,sd)/n^(.2))


for (i in 1:nw){
  v[i]<-ccdfv(ws<-ws[i],w<-w,hw<-hw,
  x1c<-x1c,x1cs[i]<-x1cs[i],hx1<-hx1,
  x2d<-x2d,x2ds<-x2ds[i],lamdx2<-lamdx2,
  x3o<-x3o,x3os<-x3os[i],lamdx3<-lamdx3)
}
...
# race,sex,region,insure,age,education
# race 1-white, 2-black, 3-Hispanic, 4-other;
# sex 1-female, 0-male
# region 1- 2- 3- 4- 
# insure 1-insured 0-uncovered
# age 0 18-24, 1 25-44, 2 45-64. 
# education 0 no education, 1 , 2 , 3 graduate
